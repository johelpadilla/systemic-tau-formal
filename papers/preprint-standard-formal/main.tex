...
\title{%
  ...
  and open scrutiny program (v0.1.8)%
}
\author{%
  ...
  \small Software version DOI: \href{https://doi.org/10.5281/zenodo.21522882}{10.5281/zenodo.21522882}\\
  \small Concept DOI: \href{https://doi.org/10.5281/zenodo.21516059}{10.5281/zenodo.21516059}\\[0.4em]
  \small \textbf{Software:} v0.1.8
  \quad·\quad
  \textbf{Preprint:} 0.1.8-r2
  \quad·\quad
...
The methodological thesis is simple: the operational core must be writable in a form that a formal verifier accepts. The monorepository \texttt{systemic-tau-formal} realizes that thesis. It exposes definitions, lemmas, and constructions in Lean~4; aligns a Python reference core; and deposits citable versions with DOIs. Version 0.1.8 under \texttt{SystemicTau/} builds with no \texttt{sorry} holes and no author-added research axioms. That is how the paradigm presents itself: as an auditable criterion.

Relative to v0.1.7, this release closes former research holes as named laboratory constructions (including a non-tent continuous return). It adds logistic scale identification and a \(C^2\)/Schwarzian package, and aligns the honesty documentation with the formal pin.
...
\begin{tikzpicture}[
  ...
  \node[box, fill=yellow!15, below=0.85cm of $(lean)!0.5!(ci)$] (zen)
    {\textbf{Zenodo} \texttt{v0.1.8}\\version DOI + concept DOI};
  \draw[arr] (lean) -- (py);
  \draw[arr] (py) -- (docs);
  \draw[arr] (docs) -- (ci);
...
\[
  ...
  \approx 4.6692016091
\]
is introduced. The fraction is a fixed ten-decimal truncation of the classical Feigenbaum constant \(\delta\) as tabulated in the period-doubling literature~\cite{feigenbaum1978,briggs1991}, encoded as numerator and denominator naturals in Lean (\texttt{feigenbaumDeltaNum}/\texttt{feigenbaumDeltaDen}). The gate prefactor is
\[
...
\begin{tikzpicture}[x=2.4cm,y=1.55cm,font=\small]
  ...
  \fill[orange!12] (0.41,-1.35) rectangle (0.50,1.45);
  ...
  \node[font=\scriptsize\bfseries,orange!70!black] at (0.455,1.25) {\rotatebox{90}{int.}};
...
\(g(\tau)\) from Equation~\eqref{eq:gate}. Note the asymmetric treatment of the
anti-synchronous ray (\(g=-1\)) relative to the positive intermediate band (\(g=0\)).}
...
The reduction module formalizes realizers of functional pairs, a laboratory continuum based on the tent map, and a continuous non-tent return \texttt{tauReturnFour} (logistically conjugate on a coherence interval). The laboratory composite package combines that return with a refined \texttt{FeigenbaumUniversal} object (operational \(\delta\)-band and quadratic tip). The bridge is the existence of citable, verified constructions---not informal narrative.

\subsection{Analytic cascade and topology}

A geometric parameter cascade with exact ratio \(\deltaop\) is defined. Its scaling ratios are proved equal to \(\deltaop\). Within the formalism, the \(\varepsilon\)--\(N\) notion of cascade limit is equivalent to a \texttt{Tendsto}-style convergence statement. Honesty blocks include toy cascades that do \emph{not} approach \(\deltaop\), so the checker distinguishes models that satisfy the scaling ratio from those that do not.

\subsection{Logistic scale identification}

A logistic module anchors a parameter sequence at landmarks \(r_0=2\), \(r_1=3\) and proves that this sequence shares scale (ratios) with the geometric cascade. It is a laboratory scale identification, expressed in Lean, between two parameter charts.

\subsection{Quadratic tip and Schwarzian}

On the logistic map, formal derivatives, a quadratic tip with \(f''\ne 0\) at the critical point, the Schwarzian inequality off the critical point, and a \texttt{FeigenbaumUniversalC2} package assembling the operational band and tip are formalized. The package places \(C^2\)/Schwarzian language inside the accepted artifact~\cite{singer1978}.

\subsection{Module inventory}

\begin{table}[ht]
\centering
\caption{Lean modules under \texttt{SystemicTau/} (v0.1.8).}
\label{tab:modules}
...
\texttt{Thresholds} & \(\sim\)180 & Core & Bands, classification, trichotomy \\
...
\texttt{Ontology} & \(\sim\)40 & Spec & Level architecture (separate layer) \\
\bottomrule
\end{tabular}
\end{table}

The \texttt{SystemicTau/} directory of version 0.1.8 builds end to end in Lean~4.14 with Mathlib~\texttt{v4.14.0}~\cite{lean4,mathlib}.
...
\begin{table}[ht]
\centering
\caption{Formal status of the v0.1.8 package under \texttt{SystemicTau/}.}
\label{tab:status}
...
\midrule
Explicitly open &
Termwise superstable logistic roots; Mathlib \(C^2\)-open renormalization /
fixed-point universality~\cite{lyubich1999};
field-derived continuum return of \(\taus\) (not a cited lab map);
unique closed form \(\tauch=f(\delta)\) beyond finite simple non-identities;
...
We have presented \texttt{systemic-tau-formal}~v0.1.8 as a Lean-accepted operational standard for Systemic Tau and RECD, together with a laboratory construction package and an open scrutiny program. The repository offers:
...
Software version associated with this text: \texttt{v0.1.8}.\\
Preprint document version: \texttt{0.1.8-r2} (2026-07-24).\\
Version DOI: \href{https://doi.org/10.5281/zenodo.21522882}{10.5281/zenodo.21522882}.\\
Concept DOI: \href{https://doi.org/10.5281/zenodo.21516059}{10.5281/zenodo.21516059}.\\
...
%------------------------------------------------------------------
\appendix
\section{Release note relative to v0.1.7}
\label{app:changelog}

Version 0.1.8 is a formal-track pin of the laboratory construction pack:

\begin{itemize}[leftmargin=1.2em]
  \item Zero \texttt{sorry} and zero author research \texttt{axiom} under \texttt{SystemicTau/}.
  \item Former open goals discharged only as named laboratory constructions.
  \item Non-tent continuous return \texttt{tauReturnFour}; refined composite package.
  \item Logistic scale identification; \(C^2\) tip and Schwarzian package.
  \item Honesty documentation aligned with the formal pin
        (\texttt{STATUS.md}, \texttt{FORMAL\_OBLIGATIONS.md},
        \texttt{FEIGENBAUM\_STATUS.md}).
\end{itemize}

\section{Expanded module map}
\label{app:modules}

Table~\ref{tab:modules-detail} expands the inventory of Section~5 for readers who want a denser map of the Lean surface. Line counts are approximate at the v0.1.8 pin.

\begin{table}[ht]
\centering
\caption{Expanded \texttt{SystemicTau/} module map (v0.1.8).}
...
\texttt{Ontology} & Spec &
  Level architecture (discourse layer; not an operational theorem). \\
\bottomrule
\end{tabularx}
\end{table}
...
J.~Padilla-Villanueva.
\newblock systemic-tau-formal: Machine-verifiable Systemic Tau (\(\taus\)) and RECD (v0.1.8).
\newblock Zenodo, 2026.
\newblock \href{https://doi.org/10.5281/zenodo.21522882}{doi:10.5281/zenodo.21522882}.
