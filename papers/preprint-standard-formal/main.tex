...
\title{%
  ...
  and open scrutiny program (v0.1.10)%
}
\author{%
  ...
  \small Preprint DOI: \href{https://doi.org/10.5281/zenodo.21537494}{10.5281/zenodo.21537494}\\
  \small Software version DOI: \href{https://doi.org/10.5281/zenodo.21537465}{10.5281/zenodo.21537465}\\
  \small Software concept DOI: \href{https://doi.org/10.5281/zenodo.21516059}{10.5281/zenodo.21516059}\\[0.4em]
  \small \textbf{Software:} v0.1.10
  \quad·\quad
  \textbf{Preprint:} 0.1.10-r2
  \quad·\quad
...
The methodological thesis is simple: the operational core must be writable in a form that a formal verifier accepts. The monorepository \texttt{systemic-tau-formal} realizes that thesis. It exposes definitions, lemmas, and constructions in Lean~4; aligns a Python reference core; and deposits citable versions with DOIs. Version 0.1.10 under \texttt{SystemicTau/} builds with no \texttt{sorry} holes and no author-added research axioms. That is how the paradigm presents itself: as an auditable criterion.

Relative to v0.1.9, this release adds termwise superstable logistic roots
(modules \texttt{FeigenbaumSuperstable}, \texttt{FeigenbaumCascade}): unique
period-1 and strict period-2 parameters; existence of a cascade-window period-4
superstable; a secondary residual zero near \(3.96\) showing full-interval uniqueness
is false; a hybrid cascade with geometric Feigenbaum tail and \(\delta\)-limit; and
conditional window uniqueness under residual monotonicity. The inverse-scale
threshold bridge from v0.1.9 is retained. Formal Rolle mono discharge, termwise
superstable roots for all \(n\), and free-\(c\) without Kendall pin remain open.
...
\begin{tikzpicture}[
  ...
  \node[box, fill=yellow!15, below=0.85cm of $(lean)!0.5!(ci)$] (zen)
    {\textbf{Zenodo} \texttt{v0.1.10}\\version DOI + concept DOI};
  \draw[arr] (lean) -- (py);
  \draw[arr] (py) -- (docs);
  \draw[arr] (docs) -- (ci);
...
\subsection{Termwise superstable roots and hybrid cascade}

Version 0.1.10 makes the logistic superstable track explicit.
Period-1 superstable is uniquely \(r=2\) on \((0,4)\).
Strict period-2 superstable is uniquely \(r=1+\sqrt5\) on \((2,4)\); the onset landmark \(r=3\) is proved \emph{not} superstable of period~2.
For period~4, the residual \(f_r^{\circ 4}(1/2)-1/2\) admits a zero in the cascade isolating window \([3.498,3.499]\) (intermediate-value theorem on an exact rational residual), yielding a strict superstable parameter there.
A second residual zero in \([3.960,3.961]\) shows that uniqueness on the full interval past period~2 is false---cascade language must name its isolating window.
Uniqueness \emph{inside} that window is proved conditionally on strict monotonicity of the residual (the classical Rolle/\(Q'>0\) input); a Horner lower-bound certificate for the classical derivative bound is recorded, while full formal discharge of residual mono remains an analysis residual.
A hybrid cascade uses true strict superstable parameters for \(n\le 2\) and Feigenbaum geometric steps for \(n\ge 3\); tail ratios equal \(\deltaop\) and therefore tend to \(\deltaop\).
The geometric tail is construction, not a termwise theorem that every hybrid term is a logistic superstable root.
...
\begin{table}[H]
\centering
\caption{Lean modules under \texttt{SystemicTau/} (v0.1.10).}
\label{tab:modules}
...
\texttt{FeigenbaumSuperstable} & \(\sim\)490 & Lab & Termwise SS roots \(n=0,1\); period-4 residual \\
\texttt{FeigenbaumCascade} & \(\sim\)540 & Lab & Window \(\exists\); hybrid \(R_n\); free-\(c\) pack \\
...
The \texttt{SystemicTau/} directory of version 0.1.10 builds end to end in Lean~4.14 with Mathlib~\texttt{v4.14.0}~\cite{lean4,mathlib}.
...
\begin{table}[H]
\centering
\caption{Formal status of the v0.1.10 package under \texttt{SystemicTau/}.}
\label{tab:status}
...
unique period-1 superstable \(r=2\); unique strict period-2 superstable \(1+\sqrt5\);
period-4 residual characterization and lower-stage exclusion;
existence of a cascade-window period-4 strict superstable (IVT);
secondary residual zero near \(3.96\) (full-interval uniqueness false);
conditional uniqueness in the cascade window under residual mono;
hybrid cascade tail ratios \(\to\deltaop\);
...
hybrid cascade \(R_n\) (true SS for \(n\le 2\), geometric Feigenbaum tail for \(n\ge 3\));
composite laboratory packages built from the above.
\smallskip\end{minipage} \\
\midrule
Explicitly open &
\begin{minipage}[t]{\linewidth}\raggedright\smallskip
Formal discharge of residual mono on the cascade window (Rolle/\(Q'\));
true termwise superstable \(R_n\) for all \(n\) (not hybrid geometric tail);
...
classical free-\(c\) derivation of \(\tauch\) without Kendall pin
(naive \(f=2/\delta\) fails machine-checked; operational inverse-scale uniqueness closed);
...
We have presented \texttt{systemic-tau-formal}~v0.1.10 as a Lean-accepted operational standard for Systemic Tau and RECD, together with a laboratory construction package and an open scrutiny program. The repository offers:
...
Software version associated with this text: \texttt{v0.1.10}.\\
Preprint document version: \texttt{0.1.10-r2} (2026-07-24).\\
Preprint DOI: \href{https://doi.org/10.5281/zenodo.21537494}{10.5281/zenodo.21537494}.\\
Software version DOI: \href{https://doi.org/10.5281/zenodo.21537465}{10.5281/zenodo.21537465}.\\
...
%------------------------------------------------------------------
\appendix
\section{Release note relative to v0.1.9}
\label{app:changelog}

Version 0.1.10 is a formal-track pin of the superstable/cascade honesty pack on top of the
v0.1.9 threshold-bridge release:

\begin{itemize}[leftmargin=1.2em]
  \item Modules \texttt{FeigenbaumSuperstable} and \texttt{FeigenbaumCascade}.
  \item Unique period-1 / strict period-2 superstable roots; period-4 cascade-window existence.
  \item Secondary residual zero \(\sim 3.96\); conditional window uniqueness under mono.
  \item Hybrid \(R_n\) with geometric tail and \(\delta_n\to\deltaop\); free-\(c\) operational pack retained.
  \item Still zero \texttt{sorry} and zero author research \texttt{axiom}.
  \item Retained from v0.1.9: inverse-scale \(f(\delta)\), ontology L0--L3, non-tent return, Schwarzian pack.
\end{itemize}

\section{Expanded module map}
\label{app:modules}

Table~\ref{tab:modules-detail} expands the inventory of Section~5 for readers who want a denser map of the Lean surface. Line counts are approximate at the v0.1.10 pin.

\begin{table}[H]
\centering
\caption{Expanded \texttt{SystemicTau/} module map (v0.1.10).}
...
\texttt{FeigenbaumSuperstable} & Lab &
  Termwise SS: unique \(r=2\); unique strict \(1+\sqrt5\); honesty \(3\not=\mathrm{SS}_2\);
  period-4 residual characterization and lower-stage exclusion. \\
\texttt{FeigenbaumCascade} & Lab &
  Cascade-window period-4 existence; secondary residual \(\sim 3.96\);
  conditional mono uniqueness; hybrid \(R_n\); free-\(c\) operational pack. \\
...
J.~Padilla-Villanueva.
\newblock systemic-tau-formal: Machine-verifiable Systemic Tau (\(\taus\)) and RECD (v0.1.10).
\newblock Zenodo, 2026.
\newblock \href{https://doi.org/10.5281/zenodo.21537465}{doi:10.5281/zenodo.21537465}.

\bibitem{preprint-standard-formal}
J.~Padilla-Villanueva.
\newblock A machine-checkable operational standard for Systemic Tau \(\taus\) and the Discrete Extramental Clock (RECD) (preprint 0.1.10-r2).
\newblock Zenodo, 2026.
\newblock \href{https://doi.org/10.5281/zenodo.21537494}{doi:10.5281/zenodo.21537494}.
